...
// General
-(void)gotPosition:(CLLocationCoordinate2D)coordinates; // Called when we successfully got the users position for the first time
@end
...
*/
-(void)getCoordinatesFromAddress:(NSString *)address returnBlock:(void(^)(CLLocationCoordinate2D coordinates, BOOL success))callback returnedAddress:(void (^)(NSString *returnString))returnedAddress;
...
/*
 This method starts to update the users position
*/
-(void)startUpdatingLocation;

/*
 This method stops to update the users position
*/
...
@property (nonatomic, assign, readonly) CLLocationCoordinate2D currentCoordinates;
