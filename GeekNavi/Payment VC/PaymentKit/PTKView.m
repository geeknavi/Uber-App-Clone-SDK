...
#define kPTKViewCardExpiryFieldStartX 55 + 180
#define kPTKViewCardCVCFieldStartX 100 + 100
#define kPTKViewCardZipFieldStartX 60 + 100

#define kPTKViewCardExpiryFieldEndX 65
#define kPTKViewCardCVCFieldEndX 130
#define kPTKViewCardZipFieldEndX 175
...
@property (nonatomic) PTKAddressZip *cardZip;
@end
...
- (void)setupCardZipCode
{
    self.cardZipField = [[PTKTextField alloc] initWithFrame:CGRectMake(kPTKViewCardZipFieldStartX, 0, 50, 20)];
    self.cardZipField.delegate = self;
    self.cardZipField.placeholder = @"ZIP";
    self.cardZipField.keyboardType = UIKeyboardTypeNumberPad;
    self.cardZipField.textColor = DarkGreyColor;
    self.cardZipField.font = DefaultBoldFont;
    
    [self.cardZipField.layer setMasksToBounds:YES];
}
...
- (PTKAddressZip *)cardZip
{
    return [PTKAddressZip addressZipWithString:self.cardZipField.text];
}
    ...
    if (!_isInitialState) {
        ...
        [UIView animateWithDuration:0.400
                         ...
                         animations:^{
                             ...
                             self.cardZipField.frame = CGRectMake(kPTKViewCardZipFieldStartX,
                                                                  self.cardZipField.frame.origin.y,
                                                                  self.cardZipField.frame.size.width,
                                                                  self.cardZipField.frame.size.height);
                         }
                         completion:^(BOOL completed) {
                             [self.cardExpiryField removeFromSuperview];
                             [self.cardCVCField removeFromSuperview];
                             [self.cardZipField removeFromSuperview];
                         }];
    }
...
- (void)stateMeta
{
    ...
    [UIView animateWithDuration:0.400 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        self.cardCVCField.frame = CGRectMake(kPTKViewCardCVCFieldEndX,
                                             ...
                                             self.cardCVCField.frame.size.width,
                                             self.cardCVCField.frame.size.height);
        self.cardZipField.frame = CGRectMake(kPTKViewCardZipFieldEndX,
                                             self.cardCVCField.frame.origin.y,
                                             self.cardCVCField.frame.size.width,
                                             self.cardCVCField.frame.size.height);
    ...
    [self.innerView addSubview:self.cardZipField];
...
- (PTKCard *)card
{
    ...
    card.addressZip = [self.cardZip string];
...
- (PTKTextField *)firstInvalidField;
{
    ...
    else if (![[PTKAddressZip addressZipWithString:self.cardZipField.text] isValid])
        return (id)self.cardZipField;
