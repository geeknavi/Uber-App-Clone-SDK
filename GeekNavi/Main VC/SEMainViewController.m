...
#import <GeekNavi/GeekDrawPresent.h>
...
@interface SEMainViewController ()<GeekMapHelperDelegate,GeekWaterViewDelegate,MKMapViewDelegate,UITextFieldDelegate,HTHorizontalSelectionListDataSource,HTHorizontalSelectionListDelegate,MFMessageComposeViewControllerDelegate,UIAlertViewDelegate,GMSAutocompleteViewControllerDelegate>{
...
- (void)viewDidLoad {
    ...
    [geekHelper startUpdatingLocation];
    ...
    // Draw the "Sedan/Sport/Limo" selectors
    [self drawSelectors];
    
    // Add the observers
    [self addObservers];
    
    // Check application state (if the user has applied before & the user needs to update something)
...
#pragma mark - Got position (delegate method)
-(void)gotPosition:(CLLocationCoordinate2D)coordinates{
    if (!geekHelper->originPinPoint) {
...
-(void)getLocationAndAddPin{
    [geekHelper addOriginPin:geekHelper.currentCoordinates mapview:mainMap];
    [geekHelper zoomToCoordinate:geekHelper.currentCoordinates withMap:mainMap animated:NO];
    
    [geekHelper getAddressFromCoordinates:geekHelper.currentCoordinates returnBlock:^(NSString *address, BOOL success) {
...
-(void)failedToUpdateAuthorizationStatus:(NSError *)error{
    if (error) {
        showAlertViewWithTitleAndMessage(nil,NSLocalizedString(@"location_denied", nil));
    }
}
...
- (IBAction)navigationAction:(id)sender {
    if (mainMap.annotations.count == 0) {
        ...
        [geekHelper moveActivePin:geekHelper.currentCoordinates mapview:mainMap];
    }

    [geekHelper zoomToCoordinate:geekHelper.currentCoordinates withMap:mainMap animated:YES];
...
- (IBAction)setPickupAction:(id)sender {
    if (!userInformation[@"vToken"] || [userInformation[@"vToken"] length] == 0) {
        showAlertViewWithTitleAndMessage(nil,@"Please add a creditcard before continuing");
        return;
    }
...
-(IBAction)bookAction:(id)sender{
    ...
    [UIView animateWithDuration:.4 animations:^{
        ...
        if (finished) {
            ...
            [bookingTint setBackgroundColor:SUB_THEME_COLOR];
                ...
                [GeekNavi bookRideWithDetails:geekHelper->originPinPoint.coordinate.latitude fromLongitude:geekHelper->originPinPoint.coordinate.longitude toLatitude:geekHelper->destinationPoint.coordinate.latitude toLongitude:geekHelper->destinationPoint.coordinate.longitude fromAddress:geekHelper->originPinPoint.title toAddress:geekHelper->destinationPoint.title totalCost:totalPrice block:^(id JSON, WebServiceResult geekResult) {
                    if (geekResult==WebServiceResultSuccess) {
                        if ([JSON[@"message"]isEqualToString:@"No active drivers"]) {
                            [self backToStartingPoint];
                            showAlertViewWithTitleAndMessage(nil,NSLocalizedString(@"no_active_drivers", nil));
                        }else{
                            ...
                            if (JSON[@"rideID"]) {
                                ...
                                showAlertViewWithTitleAndMessage(nil,@"Something went wrong.. Try again later!");
                            }
                        }
                    }
                }];
...
-(IBAction)cancelAction:(id)sender{
    ...
    [GeekNavi cancelRideWithRideID:currentRideID block:^(id JSON, WebServiceResult geekResult) {
        if (geekResult==WebServiceResultSuccess) {
            ...
            showAlertViewWithTitleAndMessage(nil,@"Something went wrong.. Please try again!");
        }
    }];
}
...
-(void)rideStatusUpdates:(NSString *)currentStatus statusCode:(int)statusCode{
    ...
    */
            ...
            [GeekNavi getDriverInformationFromRideID:currentRideID block:^(id JSON, WebServiceResult geekResult) {
                if (geekResult==WebServiceResultSuccess) {
                    if (JSON[@"data"])  {
                        ...
                        showAlertViewWithTitleAndMessage(nil,@"Something went wrong.. Please try again later!");
                        [self backToStartingPoint];
                    }
                }
            }];
    ...
    if (statusCode == 2){ // The driver canceled the ride
        showAlertViewWithTitleAndMessage(nil,@"The Ride has been canceled");
        [self backToStartingPoint];
    ...
    if (driverMode == YES) {
        ...
        [GeekNavi driverOnline:NO block:^(id JSON, WebServiceResult geekResult) {
            if (geekResult==WebServiceResultSuccess) {
                ...
                [btnRight setImage:[GeekDrawSteeringWheel imageOfWheelWithRect:btnRight.frame color:SUB_THEME_COLOR] forState:UIControlStateNormal];
        ...
        [GeekNavi driverOnline:YES block:^(id JSON, WebServiceResult geekResult) {
            if (geekResult==WebServiceResultSuccess) {
                ...
                UIButton *btnRight = [UIButton buttonWithType:UIButtonTypeCustom];
                [btnRight setFrame:CGRectMake(0, 0, 35, 35)];
                [btnRight setImage:[GeekDrawSteeringWheel imageOfWheelWithRect:btnRight.frame color:[UIColor greenColor]] forState:UIControlStateNormal];
                [btnRight addTarget:self action:@selector(initializeDriverMode) forControlEvents:UIControlEventTouchUpInside];
    ...
    [GeekNavi acceptRide:currentRideID block:^(id JSON, WebServiceResult geekResult) {
        if (geekResult==WebServiceResultSuccess) {
            ...
            if(![driverNavigateButton isDescendantOfView:self.view]){
                driverNavigateButton = [[UIButton alloc] initWithFrame:CGRectMake(8, 16, self.view.frame.size.width/2-16, 50)];
                [driverNavigateButton setBackgroundColor:SUB_THEME_COLOR];
            ...
            if(![driverArrivedButton isDescendantOfView:self.view]){
                driverArrivedButton = [[UIButton alloc] initWithFrame:CGRectMake(self.view.frame.size.width/2+8, 16, self.view.frame.size.width/2-16, 50)];
                ...
                [driverArrivedButton setTitleColor:SUB_THEME_COLOR forState:UIControlStateNormal];
                ...
                driverArrivedButton.layer.borderColor = SUB_THEME_COLOR.CGColor;
                driverArrivedButton.layer.borderWidth = 1.0f;
                [self.view addSubview:driverArrivedButton];
            }
        }else{
            showAlertViewWithTitleAndMessage(nil,@"Someone else accepted this Ride before you!");
    ...
    [GeekNavi markAsArrived:currentRideID block:^(id JSON, WebServiceResult geekResult) {
        if (geekResult==WebServiceResultSuccess) {
            ...
            */
            ...
            [geekHelper calculateDistanceBetweenCoords:geekHelper.currentCoordinates toCoords:geekHelper->destinationPoint.coordinate block:^(double miles) {
...
-(IBAction)navigateToPin:(id)sender{
    for (id obj in [mainMap annotations]) {
        if (obj == geekHelper->originPinPoint) {
            NSString *urlString = [NSString stringWithFormat:@"https://maps.apple.com/?daddr=%f,%f&saddr=%f,%f",geekHelper->originPinPoint.coordinate.latitude,geekHelper->originPinPoint.coordinate.longitude,geekHelper.currentCoordinates.latitude,geekHelper.currentCoordinates.longitude];
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:urlString]];
            break;
        }else if(obj == geekHelper->destinationPoint){
            NSString *urlString = [NSString stringWithFormat:@"https://maps.apple.com/?daddr=%f,%f&saddr=%f,%f",geekHelper->destinationPoint.coordinate.latitude,geekHelper->destinationPoint.coordinate.longitude,geekHelper.currentCoordinates.latitude,geekHelper.currentCoordinates.longitude];
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:urlString]];
            break;
        }
    }
...
- (MKOverlayRenderer *)mapView:(MKMapView *)mapView rendererForOverlay:(id<MKOverlay>)overlay{
    ...
    renderer.strokeColor = SUB_THEME_COLOR;
...
-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    GMSAutocompleteViewController *gmsController = [[GMSAutocompleteViewController alloc] init];
    gmsController.delegate = self;
    [self presentViewController:gmsController animated:YES completion:nil];
    return NO;
}

#pragma mark - Google Address Search (GMS)
/*
 If you don't want to use Google, simply use the method below;
*/
...
didAutocompleteWithPlace:(GMSPlace *)place {
    [geekHelper moveActivePin:place.coordinate mapview:mainMap];
    [geekHelper zoomToCoordinate:place.coordinate withMap:mainMap animated:YES];
    
    // Set textfield
    fromAddressTextField.text = place.formattedAddress;
...
-(void)drawSelectors{
    if(![vehicleTypeList isDescendantOfView:self.view]){
        ...
        [vehicleTypeList setTitleColor:SUB_THEME_COLOR forState:UIControlStateNormal];
        [vehicleTypeList setSelectionIndicatorColor:SUB_THEME_COLOR];
        [vehicleTypeList setSelectionIndicatorStyle:HTHorizontalSelectionIndicatorStyleBottomBar];
        [vehicleTypeList setBackgroundColor:MAIN_THEME_COLOR];
...
-(void)customizeScreen{
    ...
    if ([userInformation[@"vDriverorNot"] isEqualToString:@"driver"]) {
        ...
        [btnRight setImage:[GeekDrawPresent imageOfPresentWithRect:CGRectMake(0, 0, 30, 33) color:[UIColor darkGrayColor]] forState:UIControlStateNormal];
